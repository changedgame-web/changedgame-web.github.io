...
//    - Release
// v2.0 (5.11.2025)
//    - Add key event compatibility
// ===============================

/*:
 * @plugindesc (v.2.0)[HAMSTRUNG] Mobile controls: Joystick and Buttons
 * @author Pheonix KageDesu, Dr. G
 * @target MV
 ...
 *  PKD_MobileControls.simulateCancel(); - simulate Esc key press
 ...
 *
  * This is the HAMSTRUNG plugin version and has some restrictions:
 *    - Plugin usage is allowed only in Non-Commercial projects
 *    - Other restriction removed. It's JS.
 * 
 *  The PRO version of the plugin doesn't have these restrictions!

 *
 * Visit plugin web page for more information, also you can find Demo project.
 * 
 * If you like my Plugins, want more and often updates,
 ...
 *
 * @param keyButton
 * @text Input Key invoke
 * @desc name of the input key to press and release with the button, no value here presses no key with the button
 * @default 
 *
*/
...
//new: custom IMG path so I can keep the controller images separate
var PKD_MobileControls_IMG = "img/controller/"
...
(function () {
	...
	//what's the difference between THIS input extension and the other one for touch controls?
	//this looks like it just allows for any key to be pressed, not just the ones in Input.keyMapper (rpg_core.js)
	//if it IS in Input.keyMapper, then the methods return and leave it to the parent to do the handling.
	//otherwise, this one handles it

	//TODO: Gamepad support
	//these create mappings for keyboard keys not already mapped by Input.keyMapper
	Input.KeyMapperPKD = {};
	//Numbers
	for (i = j = 48; j <= 57; i = ++j) {
	...
	(function () {
		...
		Input._onKeyDown = function (event) {
			...
			if (Input.keyMapper[event.keyCode]) {
				console.log("Keydown current state: " + Input._currentState);
				return;
			}
		...
		Input._onKeyUp = function (event) {
			...
			if (Input.keyMapper[event.keyCode]) {
				console.log("Keyup current state: " + Input._currentState);
				return;
			}
		...
		//not sure why this one is needed. it's called nowhere else
		//?NEW
		Input.isCancel = function () {
			return Input.isTriggered('cancel') || TouchInput.isCancelled();
		};

		//convert touch location into a KDCore point (also called nowhere else)
		//?NEW
		...
		class BitmapSrc {
			...
			static LoadFromIconIndex(iconIndex) {
				...
				if (BitmapSrc.CACHE[iconIndex] == null) {
					...
					iconset = ImageManager.loadBitmap(PKD_MobileControls_IMG + "system/", filename, null, false);
			...
			static LoadFromImageFolder(filename) {
				var bs;
				bs = new BitmapSrc();
				bs.bitmap = ImageManager.loadBitmap(PKD_MobileControls_IMG + "pictures/", filename, null, true);
		...
		loadImage(filename, smooth) {
			...
			try {
				if (filename) {
					...
					filename = path.last();
					path = path.first() + '/';
					return ImageManager.loadBitmap(PKD_MobileControls_IMG + path, filename, 0, smooth || true);
				} else {
	...
	KDCore.ParamLoader = class ParamLoader {
		...
		// * Checking if plugin parameters are loaded
		isLoaded() {
			return (this.paramsRaw != null) && this.paramsRaw.hasOwnProperty(this.pluginName);
		}

		// * Parameter name without key
		isHasParameter(paramName) {
			return this.params[paramName] != null;
		}


		// * Revolves the value of the parameter (def - on default, if not nayden)
		...
		// * These keys should go after the parameter name through:
		// * Example: @param ShowDelay:int, @param TestBool:bool
		// * Text parameters that need to be returned as is can be done without a type (text, file, combo, ...)
			...
			try {
				switch (type) {
					...
					case "s":
						...
						break;
					case "structA": // * array of structures
						return JsonEx.parse(item).map((e) => {
					...
					case "strA":
						...
						break;
					case "note": // * if there are several lines in the text
	...
	(function () {
		...
		_.isPointInScreen = function (point, margin = 10) {
			...
			// * Border from the edges of the screen
			screenMargin = margin;
			if (x < screenMargin) {
				return false;
			}
		...
		// * Asynchronous image loading, returns bitmap when loaded
		// * Example of using loadImageAsync(a, b).then(method)
		// bitmap will be passed to the method as the first argument
		_.loadImageAsync = async function (folder, filename) {
			var promise;
			promise = new Promise(function (resolve, reject) {
				var b;
				b = ImageManager.loadBitmap(PKD_MobileControls_IMG + folder + "/", filename);
	...
	KDCore.TimedUpdate = class TimedUpdate {
		...
		// * Randomize interval @interval (-min, +max)
	...
	KDCore.Button = class Button extends Sprite {
		constructor() {
			...
			this._clickHandlers = [];
			this._buttonPressHandlers = [];
			this._buttonReleaseHandlers = [];
			this._manualHided = false;
			this._manualDisabled = false;
			this._condition = null; // * Condition for Visible
			this._condition2 = null; // * Condition for Enable\Disable
		...
		addButtonPressHandler(method) {
			return this._buttonPressHandlers.push(method);
		}

		clearButtonPressHandlers() {
			return this._buttonPressHandlers = [];
		}

		addButtonReleaseHandler(method) {
			return this._buttonReleaseHandlers.push(method);
		}

		clearButtonReleaseHandlers() {
			return this._buttonReleaseHandlers = [];
		}
		...
		//what's the difference between this one and the other, _updateMouseClick?
		//when is this one ever called?
		updateMouseClick() {
			if (!this.isActive()) {
				this._unTouch();
				return;
			...
			//this runs on release if released inside the button
			if (this._touching === true) {
				if (TouchInput.isReleased() || !this.touchInButton()) {
					this._unTouch();
					this.callButtonReleaseHandler();
		...
		//called on button release inside of the button perimeter
		callClickHandler() {
		...
		callButtonPressHandler() {
			if (this._buttonPressHandlers.length > 0) {
				return this._buttonPressHandlers.forEach(function (method) {
					return method();
				});
			}
		}
		callButtonReleaseHandler() {
				if (this._buttonReleaseHandlers.length > 0) {
					return this._buttonReleaseHandlers.forEach(function (method) {
					return method();
				});
			}
		}
		...
		static FromSet(imgName, sourceFolder = null) {
			...
			getterFunc = function (filename) {
				...
				return ImageManager.loadBitmap(PKD_MobileControls_IMG + "pictures/", filename, null, true);
			};
			if (sourceFolder != null) {
				getterFunc = function (filename) {
					return ImageManager.loadBitmap(PKD_MobileControls_IMG + sourceFolder + "/", filename);
				};
			}
	...
	KDCore.Sprite = (function (superClass) {
		...
		class Sprite extends superClass {
			...
			// * Loader from plugin parameters (safe)
	...
	//------------------------------------------------------------------------------
	//@[AUTO EXTEND]
	// * Button Mini - simplified class Sprite_XButton (KDCore.Button)

	// * Takes the name of the button image file без _00
	// * Image titles must be in standard format _00, _01, [_03]
	// * _02 - not used in this class

	// * The class uses a global temporary variable to determine whether the mouse is in the button area

	// * If isFull is true, then you need _03
	KDCore.ButtonM = class ButtonM extends KDCore.Sprite {
		...
		//these 4 are new
		addButtonPressHandler(method) {
			this._buttonPressHandler = method;
		}

		clearButtonPressHandler() {
			this._buttonPressHandler = null;
		}

		addButtonReleaseHandler(method) {
			this._buttonReleaseHandler = method;
		}

		clearButtonReleaseHandler() {
			this._buttonReleaseHandler = null;
		}


		// * Reproduces the visual effect of pressing
		simulateClick() {
			if (!this.isActive()) {
				return;
			}
		...
		buttonPress() {
			if (this._buttonPressHandler != null) {
				return this._buttonPressHandler();
			}
		}
		buttonRelease() {
			if (this._buttonReleaseHandler != null) {
				return this._buttonReleaseHandler();
			}
		}
		...
		update() {
			...
			//runs one way or another, this._updateMouseClick()
			var result = this._updateMain();

			//console.log("Input current state: " + Input._currentState);
	...
	(function () {
		...
		_._updateHover = function () {
			...
			// * so that the pressing effect does not stop
		...
		//this think looks EXACTLY like updateMouseClick(), so why do we have both?
		...
		// * Now when you press any button, the player will not walk on the map
	...
	// * Button Mini User - a class with a file definition for each state separately
	// * Accepts the same arguments, only instead of a file name, three images (names)
...
// * Loading and processing of plugin parameters
(function () {

	//load parameters and register plugin commands that can be called from the GUI-friendly control flow.
	//commands: show, hide, enable, disable
	...
	//the only one I really need to look at here; I'm using this with MV...
	RegisterPluginCommandsMV = () => {
...
(function () {
	...
	Input._signY = function () {
		...
		//we now use simulated arrow key presses; we don't need to check the standard joypad direction
...
(function () {
	...
	Sprite_UIElement = (function () {
		// * ABSTRACT means that the class itself does not create anything, does not store data
		//@[ABSTRACT]
		class Sprite_UIElement extends KDCore.Sprite {
			...
			// * General method (all elements have it)
			// * Calls drawText by default, but children can override
			draw() {
			...
			// * If initially invisible (from the parameters), then not active at all
			isActive() {
			...
			// * General method (can you edit visually)
			isCanBeEdited() {
				return false;
			}

			// * General method (should I hide during game messages)
			isHaveHideWithMessageFlag() {
				return false;
			}

			// * General method (whether the object is under the mouse)
			isUnderMouse() {
				var ref;
				return (ref = this.zeroChild()) != null ? ref.isUnderMouse() : void 0;
			}

			// * Parameters of the first element (if any)
			realWidth() {
				var child;
			...
			// * The first "physical" element (sprite)
			zeroChild() {
				return this.children[0];
			}

			// * Method for restoring the value to standard settings
			reset(property) {
				var e;
				try {
					switch (property) {
...
(function () {  //╒═════════════════════════════════════════════════════════════════════════╛
	...
	_._init = function () {
		...
		try {
			...
			e = error;
			KDCore.warning(e);
			// * If an error has occurred during the creation, turn off the element
	...
	// * Preparation of the element (checking parameters)
	_._prepare = function () {
		if (this.params == null) {
	...
	// * Heirs create their elements in this method
...
Sprite_PKDJoystick = class Sprite_PKDJoystick extends PKD_MobileControls.Sprite_UIElement {
	...
	constructor() {
		super();
		this.lastPos = 0; //keep track of old position
		this.moveOnPosition();
	}
...
(function () {
	...
	_.update = function () {
		...
		//if joystick is pulled out to edge and config says we sprint in this state, we set dashing to true
		if (this.params.dashingOnEdge === true) {


			//simulate shift key press only if the state has changed
			if(Input.mJoyDash != this._isRunning) {
				Input._currentState['shift'] = this._isRunning;
			}
	...
	_._createPartsBase = function () {
		//ImageManager.loadBitmap(PKD_MobileControls_IMG + "pictures/", filename, null, true);
		...
		this.base = new Sprite(ImageManager.loadBitmap(PKD_MobileControls_IMG + "pictures/", 'JoystickBase_00', null, true));
		this.baseFore = new Sprite(ImageManager.loadBitmap(PKD_MobileControls_IMG + "pictures/", 'JoystickBase', null, true));
		...
		this.main = new Sprite(ImageManager.loadBitmap(PKD_MobileControls_IMG + "pictures/", 'JoystickMain_00', null, true));

		this.main.anchor.x = 0.5;
		this.main.anchor.y = 0.5;

		//this.mainFore = new Sprite(ImageManager.loadPicture('JoystickMain'));
		this.mainFore = new Sprite(ImageManager.loadBitmap(PKD_MobileControls_IMG + "pictures/", 'JoystickMain', null, true));
	...
	_._createPartsArrows = function () {
		...
		this.base = new Sprite(ImageManager.loadBitmap(PKD_MobileControls_IMG + "pictures/", 'JoystickBase_Arrows_00', null, true));
		this.baseFore = new Sprite(ImageManager.loadBitmap(PKD_MobileControls_IMG + "pictures/", 'JoystickBase_Arrows', null, true));
		...
		this.arrowHighlitghts = [
			...
			ImageManager.loadBitmap(PKD_MobileControls_IMG + "pictures/", 'JoystickBase_Arrows_D', null, true),
			null,
			//ImageManager.loadPicture('JoystickBase_Arrows_L'), // 4
			ImageManager.loadBitmap(PKD_MobileControls_IMG + "pictures/", 'JoystickBase_Arrows_L', null, true),
			null,
			//ImageManager.loadPicture('JoystickBase_Arrows_R'), // 6
			ImageManager.loadBitmap(PKD_MobileControls_IMG + "pictures/", 'JoystickBase_Arrows_R', null, true),
			null,
			//ImageManager.loadPicture('JoystickBase_Arrows_U') // 8
			ImageManager.loadBitmap(PKD_MobileControls_IMG + "pictures/", 'JoystickBase_Arrows_U', null, true),
		];
		this.base.bitmap.addLoadListener(this._afterCreateParts.bind(this));
		return this._afterCreateParts();
	};
	...
	_._resetMoving = function () {
		...
		this.lastPos = 0;
		//we're OK to put this here; it's only called when the joystick is released
		{
	...
	_._onMoved4 = function () {
		...
		//only update keypresses on joystick location change
		//Input._currentState[buttonName] = true;
		if(section != this.lastPos) {
			this.lastPos = section;
			...
			switch (section) {
				case 2: // * DOWN
					...
					//don't need to clone the _onKeyDown method because we know these keys aren't special case ones
					{
						Input._currentState['up'] = false;
	...
	_.highlightArrowSectionFor4 = function (sectionNumber) {
		...
		if (sectionNumber < 0) {
			...
			this.baseFore.bitmap = ImageManager.loadBitmap(PKD_MobileControls_IMG + "pictures/", 'JoystickBase_Arrows', null, true);
...
// Generated by CoffeeScript 2.5.1
// * User button on the screen
(function () {
	var Sprite_UIUButton;
	// * Button on the screen, you can press
	Sprite_UIUButton = class Sprite_UIUButton extends PKD_MobileControls.Sprite_UIElement {
		...
		// * The button does not support the redrawing
		draw() { } // * EMPTY
		...
		// * Just a challenge of the method
		call() {
			var ref;
			return (ref = this.button) != null ? ref.click() : void 0;
		}

		// * Calling the method with simulation of pressing
		click() {
			var ref, ref1;
...
(function () {  //╒═════════════════════════════════════════════════════════════════════════╛
	...
	_._createContent = function () {
		if (this.params.states.main.isEmpty()) {
			KDCore.warning('You tried to create a Button without Main Image');
			return;
		}
		this.button = new KDCore.ButtonMU(this.params.states, true, this.rootImageFolder());
		// * So that it is impossible to click
		...
		this._registerKeyboardButtonMethods();
	};
	_._registerClickMethod = function () {
		...
		method = null;
		try {
			// * If the number, then the general event means
	...
	_._registerKeyboardButtonMethods = function() {

		var ref = this;
		
		//improper format; return early
		if(!String.any(ref.params.keyButton)) {
			return;
		}

		var method = function() {

			var buttonName = ref.params.keyButton;

			//clone of Input._onKeyDown
		...
		this.button.addButtonPressHandler(method);

		var rMethod = function() {
			
			var buttonName = ref.params.keyButton;

			//clone of Input._onKeyUp
			if (buttonName) {
				Input._currentState[buttonName] = false;
			}
			console.log("Release");
		}
		this.button.addButtonReleaseHandler(rMethod);
...
(function () {
	...
	//@[ALIAS]
	// * Creating a mobile control interface
	...
	terminate() {
		...
		for (i = 0, len = ref.length; i < len; i++) {
			...
			if (e != null) {
				...
				//release held state(s)
				if(e instanceof PKD_MobileControls.Sprite_UIUButton ) {
					var buttonName = e.params.keyButton;
					if (buttonName) {
						Input._currentState[buttonName] = false;
					}
				}

				if(e instanceof Sprite_PKDJoystick) {
					{
						Input._currentState['up'] = false;
						Input._currentState['down'] = false;
						Input._currentState['left'] = false;
...
(function () {  //╒═════════════════════════════════════════════════════════════════════════╛
	...
	_._init = function () {
		...
		//ignore if set to do so.
		if(ConfigManager.showOnscreenControls != null) {
			if(ConfigManager.showOnscreenControls == false) {
				return;
			}
		}
		// //hide all buttons
		// //(alt method; does the same as the method above; we can't alter visibility after this configuration ends because this clones settings)
		// for(var i = 0; i < PKD_MobileControls.BUTTONS.length; ++i) {
		// 	PKD_MobileControls.BUTTONS[i].visible = false;
		// }
		// //hide joystick (is our joystick count limited to 1?)
		// PKD_MobileControls.JOYSTICK.visible = false;
		...
		if (PKD_MobileControls.JOYSTICK.visible === true) {
			...
			// * The buttons should start with 1, so we still take first place
	...
	_._createButtons = function () {
		//same as what's in MV plugin builder, but de-obfuscated
		var buttonDataList = PKD_MobileControls.BUTTONS;
		for (var i = 0; i < buttonDataList.length; i++) {
			...
			var buttonSprite = new PKD_MobileControls.Sprite_UIUButton(buttonData);
			
			//store and display it
			this.elements.push(buttonSprite);
...
//I can't find this on the internet... strange.
...
//new tack-on hacks: show and hide the entire UI dynamically


//will show or hide a single control
//gameSystem stores the control settings ONLY. 
//the buttons themselves seem to be part of Scene_Map, and are only put there after the modified method createSpriteset() us called

//Scene_Manager -> Scene_Map -> Spriteset_MobileControls -> children
//SceneManager._scene._mcUI.children


//we could modify the init function to just ignore adding the controls if we disable them...
...
//PKD_MobileControls.mcui.children[0].show();




//might need to alter this option to enable/disable screen touching if onscreen buttons are disabled:
//PKD_MobileControls.MAP_TOUCH



//player uses this method to move:
//Game_Player.prototype.moveByInput



//user button detection method:
//Input.isPressed(this._params[1]);
//need to make this respond to touch input

//Input._signX and signY are how we transfer joystick values into player movement
